...
. Observe the logs that are generated root endpoint several times in your browser and tailing the logs.  Allow this process to run through the next few steps.
+
[source.terminal]
----
...
. Turn logging down.  In your `app-config` repository, edit the `greeting-config.yml`.  Set the log level to `INFO`.  Make sure to push back to Github.
+
[source,yaml]
----
logging:
