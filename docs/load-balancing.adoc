...
Observe the PCF discovery service dashboard and you'll notice that each microservice is registering using its direct ip address.  This allows Ribbon to effectively load-balance directly across the running fortune-service instances without going through the PCF GoRouter.
