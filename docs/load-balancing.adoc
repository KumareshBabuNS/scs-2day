...
Refresh the `greeting-ribbon-rest` root endpoint.  If `fortune-service` is still scaled at three instances, you should see Ribbon load balance requests across these instances.
