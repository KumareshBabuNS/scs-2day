...
[NOTE]
====
PCF version 1.10 has a new feature called direct "container-to-container" networking, which provides a mechanism for an operator to configure which cloudfoundry applications are allowed to call other cloudfoundry applications directly (without going through the cloudfoundry GoRouter).

You can read about this feature in this https://content.pivotal.io/blog/building-spring-microservices-with-cloud-foundrys-new-container-networking-stack[excellent blog post^] by Chris Sterling.
...
. Once the plugin is installed, invoke the following command to allow direct communication from the greeting-service application to the fortune-service application:

[source.terminal]
----
cf allow-access greeting-service fortune-service --protocol tcp --port 8080
----
====
